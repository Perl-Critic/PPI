...
MULTIPLE_PACKAGES: {
	...
	my @stmts = $doc->schildren;
	is( $stmts[1]->namespace, 'Foo', 'between Foo and Bar is Foo' );
	is( $stmts[3]->namespace, 'Bar', 'after Bar is Bar' );
}
...
CODE_BEFORE_ANY_PACKAGE: {
	...
	my @stmts = $doc->schildren;
	is( $stmts[0]->namespace, 'main', 'code before any package is main' );
	is( $stmts[2]->namespace, 'Foo', 'code after package Foo is Foo' );
}
...
END_PERL
	...
	is( $inner_kids[0]->namespace, 'Inner',
		'deep inside nested block-form is Inner' );
	is( $outer_kids[1]->namespace, 'Outer',
		'after inner block reverts to Outer' );

	is( $top[1]->namespace, 'main',
