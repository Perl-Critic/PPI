...
sub h {
	...
	subtest(
		...
		sub {
			my $exceptions =
			  grep { $exception{$_} } keys %{ $test->{expected} };
		  ...
		  SKIP: {
				...
				is( $document->serialize(),
					$test->{content}, 'Document serializes correctly' );
			}
			...
			is( $heredoc->{$_}, $test->{expected}{$_}, "property '$_'" )
			  for grep { !$exception{$_} } keys %{ $test->{expected} };
		}
	);
}
