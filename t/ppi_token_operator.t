...
OPERATOR_X: {
	my @tests = (
		...
		{
			desc =>
			  'preceding word looks like a force but is not',      # github #133
			code     => '$a->package x3',
			expected => [
				'PPI::Token::Symbol'     => '$a',
				'PPI::Token::Operator'   => '->',
		...
		{
			desc =>
'x plus whitespace on the left of => that is not the first token in the doc',
			code     => '1;x =>1;',
			expected => [
				'PPI::Statement'         => '1;',
				'PPI::Token::Number'     => '1',
		...
		{
			desc =>
			  'x on the left of => that is not the first token in the doc',
			code     => '1;x=>1;',
			expected => [
				'PPI::Statement'        => '1;',
				'PPI::Token::Number'    => '1',
		...
		{
			desc =>
			  'x immediately followed by => should not be mistaken for x=',
			code     => 'x=>1',
			expected => [
				'PPI::Token::Word'     => 'x',
				'PPI::Token::Operator' => '=>',
	...
	foreach my $operator ( keys %operators ) {
		...
		if ( $operator =~ /\w$/ || $operator eq '<<' )
		{    # want << operator, not heredoc
			$code .= ' ';
			push @expected, ( 'PPI::Token::Whitespace' => ' ' );
		}
		$code .= 'x3';
	...
	foreach my $noproto ( keys %noprotos ) {
		...
		push @tests,
		  {
			desc => "builtin $noproto does not force following x to be a word",
			code => $code,
			expected => \@expected
		  };
	}
