...
use Test::More tests => 594 + ( warns_on_misplaced_underscore() ? 2 : 0 ) +
  ( $ENV{AUTHOR_TESTING} ? 1 : 0 );
...
SCOPE: {
	...
	while (@examples) {
		...
		if ( warns_on_misplaced_underscore()
			and ( $code eq '1_0e1_0' or $code eq '1_0' or $code eq '1_0.' ) )
		{
		  SKIP: {
		...
		is( $token->isa('PPI::Token::Number::Float'),
			$is_float, "'$code' " . ( $is_float ? "is" : "not" ) . " ::Float" );
		is( $token->isa('PPI::Token::Number::Exp'),
			$is_exp, "'$code' " . ( $is_float ? "is" : "not" ) . " ::Exp" );

		next if $base == 256;

		$^W = 0;
		my $underscore_incompatible =
		  warns_on_misplaced_underscore() && $code =~ /^1_0[.]?$/;
		my $incomplete_incompatible =
		  dies_on_incomplete_bx() && $code =~ /^0[bx]$/;
		my $literal = eval $code;
		my $err     = $@;
		$literal = undef
		  if $underscore_incompatible || $incomplete_incompatible;
		warning_is { $literal = eval $code } "Misplaced _ in number",
		  "$] warns about misplaced underscore"
		  if $underscore_incompatible;
		like(
			$err,
			qr/No digits found for (binary|hexadecimal) literal/,
...
foreach my $code ( '08', '09', '0778', '0779' ) {
	...
	ok(
		$token->{_error} && $token->{_error} =~ m/octal/i,
...
BINARY: {
	my @tests = (
		...
		{ code => '0b1__1', error => 0, value => 3 }
		,    # perl warns, but parses it
		{ code => '0b1__1_', error => 0, value => 3 }
		,    # perl warns, but parses it
			 # Bad binary numbers
	...
	foreach my $test (@tests) {
		...
		if ( $test->{error} ) {
			ok(
				$token->{_error} && $token->{_error} =~ m/binary/i,
			...
			is( $token->literal, $test->{value},
				"literal('$code') is $test->{value}" );
		}
...
HEX: {
	my @tests = (
		...
		,    # perl warns, but parses it
		{ code => '0x__1_', parsed => '0x__1_', value => 1 }
		,    # perl warns, but parses it
		{ code => '0X1',        parsed => '0X1',        value => 1 },
	...
	foreach my $test (@tests) {
		...
		is( $token->content, $test->{parsed},
			"correctly parsed everything expected" );
		is( $token->literal, $test->{value},
			"literal('$code') is $test->{value}" );
	}
}
