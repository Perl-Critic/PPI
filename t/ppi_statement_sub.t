...
NAME: {
	for my $test (
		...
		{ code => 'sub baz : method:lvalue{}',       name => 'baz' },
		{ code => 'sub baz (*) : method : lvalue{}', name => 'baz' },
		{ code => 'sub x64 {}',                      name => 'x64' }
		,    # should not be parsed as x operator
	  )
	{
		my $code = $test->{code};
		my $name = $test->{name};
...
LEXSUB: {
	  ...
	  )
	{
		...
		my ( $sub_statement, $dummy ) = $Document->schildren();
		isa_ok( $sub_statement, 'PPI::Statement::Sub',
			"$code: document child is a sub" );
...
PROTOTYPE: {
	...
	for my $test ( [ '', undef ], [ '()', '' ], [ '( $*Z@ )', '$*Z@' ], ) {
		...
		my ( $sub_statement, $dummy ) = $Document->schildren();
		isa_ok( $sub_statement, 'PPI::Statement::Sub',
			"$proto_text document child is a sub" );
		is( $dummy, undef, "$proto_text document has exactly one child" );
		is( $sub_statement->prototype, $expected,
			"$proto_text: prototype matches" );
	}
}
...
BLOCK_AND_FORWARD: {
	  ...
	  )
	{
		...
		is(
			!$sub_statement->block,
			!!$sub_statement->forward,
			"$code: block and forward are opposites"
		);
	}
}
...
RESERVED: {
	  ...
	  )
	{
		...
		isa_ok( $sub_statement, 'PPI::Statement::Sub',
			"$code: document child is a sub" );
		is( $dummy, undef, "$code: document has exactly one child" );
		is( !!$sub_statement->reserved, !!$reserved,
			"$code: reserved matches" );
	}
}
...
sub test_sub_as {
	...
	isa_ok( $sub_statement, 'PPI::Statement::Sub',
		"$code: document child is a sub" );
	isnt( ref $sub_statement,
		'PPI::Statement::Scheduled', "$code: not a PPI::Statement::Scheduled" );
	is( $dummy, undef, "$code: document has exactly one child" );
